disease_symptoms = {
    ...
    "covid-19": "Fever or chills,Cough,Shortness of breath, difficulty breathing, Fatigue ,Muscle or body aches,Headache,New loss of taste or smell,Sore throat, runny nose,Nausea,Diarrhea"
}
...
disease_specialists = {
    ...
    "Pulmonologist": ["Pneumonia", "Tuberculosis (TB)", "Lung Disease", "Bronchitis", "Asthma", "Lung Cancer"],
...
while True:
    user_input = input("Enter a symptom (type 'end' to stop): ").strip().lower()

    if user_input == 'end':
        break

    if user_input and user_input not in user_symptoms:
        user_symptoms.append(user_input)

    predicted_diseases, disease_matches = detect_diseases(user_symptoms)

    if predicted_diseases:
        print("Current Predicted Diseases:", ", ".join(predicted_diseases))
    else:
        print("No disease matches your symptoms. Please consult a doctor.")

    if len(predicted_diseases) == 1:
        final_disease = predicted_diseases[0]
        print("Final Diagnosis:", final_disease)

risk_level=input("would you like to know about the risk level of this disease?(true,false)").strip().lower()
if risk_level=='true':
    risk_found= False
for risk, disease in risklevels_disease.items():
    if final_disease == risk:
        print(f'risk level is {disease} for {final_disease} disease.')
        risk_found= True
        break
                
if not risk_found:
    print("no specific risk level found.")

doctor_consult=input("would you like to know about the doctor?(true/false)").strip().lower()
if doctor_consult=="true":
    doctor_consult = False
    for doctors, diseases in disease_specialists.items():
        if final_disease in diseases:
            print(f"Consult a {doctors} for {final_disease}.")
            doctor_consult = True
            break
        else:
            print("No specific specialist found. Please consult a doctor.")   
